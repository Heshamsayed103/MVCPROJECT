...
namespace Company.G04.PL.Controllers
{
    public class RoleController : Controller
    {
        ...
        public async Task<IActionResult> AddOrRemoveUsers(string roleId)
        {
            ...
            var usersInRole = new List<UserInRoleViewDto>();
            var users = await _userManager.Users.ToListAsync();

            foreach (var user in users)
            {
                var userInRole = new UserInRoleViewDto()
                {
                    UserId = user.Id,
        ...
        [HttpPost]
        public async Task<IActionResult> AddOrRemoveUsers(string roleId,List<UserInRoleViewDto> users)
        {
            var role = await _roleManager.FindByIdAsync(roleId);
            if (role is null)
